...
// مدل ساده برای وضعیت چالش‌ها
const challengeSchema = new mongoose.Schema({
    userId: { type: String, required: true },
    challengeId: { type: String, required: true }, // مثل 'insta_follow'
...
// API برای تایید و ثبت امتیاز چالش
app.post('/api/complete-challenge', async (req, res) => {
    try {
        const { userId, challengeId, username } = req.body;

        // ۱. چک کردن اینکه قبلاً انجام نداده باشه
        const alreadyDone = await Challenge.findOne({ userId, challengeId });
        if (alreadyDone) {
            return res.status(400).json({ message: 'این امتیاز قبلاً دریافت شده است!' });
        }

        // ۲. ثبت در دیتابیس چالش‌ها
        const newChallenge = new Challenge({ userId, challengeId });
        await newChallenge.save();

        // ۳. ثبت یک رکورد در Coupon (یا هر جایی که امتیازات رو ذخیره می‌کنی) برای اضافه شدن به لیدربورد
        const reward = new Coupon({
            userId,
            username,
            title: 'جایزه فالو اینستاگرام',
            score: 1000,
            type: 'challenge'
        });
        await reward.save();

        res.status(200).json({ success: true, message: '۱۰۰۰ امتیاز با موفقیت اضافه شد!' });
    } catch (error) {
